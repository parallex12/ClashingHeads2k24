...
import { fetchAllChallengeRequests, updateChallengeRequestForUser } from "../../state-management/features/challengeRequests/challengeRequestsSlice";
...
const ChallengeRequests = (props) => {
  ...
  const { allRequests } = useSelector(selectChallengeClashRequests);
  ...
  const memoizedRequests = useMemo(() => {
    return activeFilter == "Sent" ? memoizedRequestsSent : memoizedRequestsRecieved
  }, [allRequests, activeFilter, user?.id]);
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          {filtersOption?.map((item, index) => {
            return <FilterItem key={index} index={index} data={item}
              count={item == "Sent" ? memoizedRequestsSent?.length : memoizedRequestsRecieved?.length}
            />;
          })}
        </ScrollView>
      </View>
        ...
        <View style={styles.content}>
          {
            memoizedRequests?.map((item, index) => {
              return <DualClashCard
                onAcceptRequest={() => onAcceptChallenge(item?.id)}
